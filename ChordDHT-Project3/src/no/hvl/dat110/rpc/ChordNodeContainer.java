...
/**
 * This is the 'server' for the node where the registry is started and where the
 * binding of the remote stub object for the Node is done. In addition, all
 * periodic chord operations are started in this class currently.
 * 
 * @author tdoy exercise/demo - dat110
 */
...
public class ChordNodeContainer {
	...
	private void createNodeContainer() throws RemoteException, UnknownHostException {
		...
		// create a registry to hold the chordnode stub and start it on the port located
		// in the StaticTracker class
		Registry registry = null;
		try {
			registry = LocateRegistry.getRegistry(StaticTracker.PORT);
			System.out.println("Registry " + registry.list()); // the method registry.list() must be invoked for the
																// exception to work
	...
	private void stabilizationProtocols() throws InterruptedException, IOException {
		...
		// Schedule updateSuccessor => run this thread periodically to set the first
		// successor pointer to the correct node
		UpdateSuccessor updatesucc = new UpdateSuccessor(chordnode);
		updatesucc.start();

		// Scheduler to stabilize ring => we run this thread periodically to stabilize
		// the ring
		...
		// Scheduler to check predecessor => schedule check predecessor to run
		// periodically
